...
import { addApiUsage, getApiUsage } from './store.js';
...
async function fetchPage(type, bgnDt, endDt, page) {
  ...
  // 응답이 실패여도 호출은 한 번 쓴 것이다. 기록에 실패해도 조회는 계속한다.
  try { await addApiUsage(usageDay(), OPERATIONS[type], 1); }
  catch (err) { console.error('사용량 기록 실패:', err.message); }
